...
void TestBlitter2D(std::string const & test_name, uint32_t array_size, uint32_t mip_levels, bool linear)
{
	...
	for (int i = 0; i < 50; ++ i)
	{
		...
		uint32_t src_x_size = std::min(dis_x(gen) + 1, std::min(w - src_x_offset, w - dst_x_offset));
		uint32_t src_y_size = std::min(dis_y(gen) + 1, std::min(h - src_y_offset, h - dst_y_offset));
		uint32_t dst_x_size = std::min(dis_x(gen) + 1, std::min(w - src_x_offset, w - dst_x_offset));
		uint32_t dst_y_size = std::min(dis_y(gen) + 1, std::min(h - src_y_offset, h - dst_y_offset));
